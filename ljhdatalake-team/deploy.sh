...
if ! $nflag
then
    STACK_NAME="ljhdatalake-$TEAM_NAME"
fi
...
# Only when deploying a Demo of LJH DATALAKE
DEMO=$(sed -e 's/^"//' -e 's/"$//' <<<"$(aws ssm get-parameter --name /LJHDATALAKE/Misc/Demo --profile ${PROFILE} --query "Parameter.Value")")
if [ "$DEMO" == "true" ];then
  echo "Checking if stack exists ..."
  STACK_CROSS_ACCOUNT_NAME=ljhdatalake-crossaccount-role-${ENV}-${TEAM_NAME}
...
CENTRAL_BUCKET=$(sed -e 's/^"//' -e 's/"$//' <<<"$(aws ssm get-parameter --name /LJHDATALAKE/S3/CentralBucket --profile $PROFILE --query "Parameter.Value")")
STAGE_BUCKET=$(sed -e 's/^"//' -e 's/"$//' <<<"$(aws ssm get-parameter --name /LJHDATALAKE/S3/StageBucket --profile $PROFILE --query "Parameter.Value")")
ANALYTICS_BUCKET=$(sed -e 's/^"//' -e 's/"$//' <<<"$(aws ssm get-parameter --name /LJHDATALAKE/S3/AnalyticsBucket --profile $PROFILE --query "Parameter.Value")")
CRAWLER_ARN=$(sed -e 's/^"//' -e 's/"$//' <<<"$(aws ssm get-parameter --name /LJHDATALAKE/IAM/$TEAM_NAME/CrawlerRoleArn --profile $PROFILE --query "Parameter.Value")")
