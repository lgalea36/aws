...
function with_backoff {
  ...
  if [[ $exitCode != 0 ]]
  then
    echo "Command ($@) failed all attempts\n Please check that IAM role ljhdatalake-cicd-team-codecommit-${ENV}-${TEAM_NAME} exists in the DevOps account" 1>&2
    exit 1
  fi
...
with_backoff aws iam get-role --role-name ljhdatalake-cicd-team-codecommit-${ENV}-${TEAM_NAME} --profile crossaccount