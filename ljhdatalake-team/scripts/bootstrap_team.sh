...
function bootstrap_team_repository()
{
  ...
  TEAM_REPOSITORY=ljhdatalake-${TEAM}-$(cut -d'-' -f2 <<<${TEMPLATE_REPOSITORY})
  set +e
...
CHILD_ACCOUNT=$(sed -e 's/^"//' -e 's/"$//' <<<"$(aws ssm get-parameter --name /LJHDATALAKE/Misc/${ENV}/AccountId --query "Parameter.Value")")
echo "Checking if stack exists ..."
STACK_NAME=ljhdatalake-crossaccount-role-${ENV}-${TEAM_NAME}
