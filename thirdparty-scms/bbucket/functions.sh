...
function bootstrap_repository_scm()
{
    ...
    git init
    if [[ "${REPOSITORY}" == "ljhdatalake-team" ]]; then #remove ljhdatalake-team repositories creation
...
function setup_bbucket_workspacevars() {
  ...
  curl -sS -X POST -H "Content-Type: application/json" -u ${BBUSER}:${APP_PASS} -d "{ \"type\": \"pipeline_variable\", \"key\": \"LJHDATALAKE_AWS_ACCESS_KEY_ID\", \"value\": \"${LJHDATALAKE_AWS_ACCESS_KEY_ID}\", \"secured\": \"true\" }" https://api.bitbucket.org/2.0/workspaces/$WORKSPACE/pipelines-config/variables > /dev/null
  curl -sS -X POST -H "Content-Type: application/json" -u ${BBUSER}:${APP_PASS} -d "{ \"type\": \"pipeline_variable\", \"key\": \"LJHDATALAKE_AWS_SECRET_ACCESS_KEY\", \"value\": \"${LJHDATALAKE_AWS_SECRET_ACCESS_KEY}\", \"secured\": \"true\" }" https://api.bitbucket.org/2.0/workspaces/$WORKSPACE/pipelines-config/variables > /dev/null
  curl -sS -X POST -H "Content-Type: application/json" -u ${BBUSER}:${APP_PASS} -d "{ \"type\": \"pipeline_variable\", \"key\": \"LJHDATALAKE_AWS_SESSION_TOKEN\", \"value\": \"${LJHDATALAKE_AWS_SESSION_TOKEN}\", \"secured\": \"true\" }" https://api.bitbucket.org/2.0/workspaces/$WORKSPACE/pipelines-config/variables > /dev/null
  curl -sS -X POST -H "Content-Type: application/json" -u ${BBUSER}:${APP_PASS} -d "{ \"type\": \"pipeline_variable\", \"key\": \"LJHDATALAKE_AWS_REGION\", \"value\": \"${REGION}\", \"secured\": \"false\" }" https://api.bitbucket.org/2.0/workspaces/$WORKSPACE/pipelines-config/variables > /dev/null
  curl -sS -X POST -H "Content-Type: application/json" -u ${BBUSER}:${APP_PASS} -d "{ \"type\": \"pipeline_variable\", \"key\": \"LJHDATALAKE_AWS_ROLE_TO_ASSUME_NAME\", \"value\": \"${LJHDATALAKE_AWS_ROLE_TO_ASSUME_NAME}\", \"secured\": \"false\" }" https://api.bitbucket.org/2.0/workspaces/$WORKSPACE/pipelines-config/variables > /dev/null
  curl -sS -X POST -H "Content-Type: application/json" -u ${BBUSER}:${APP_PASS} -d "{ \"type\": \"pipeline_variable\", \"key\": \"LJHDATALAKE_AWS_ROLE_TO_ASSUME_ARN\", \"value\": \"${LJHDATALAKE_AWS_ROLE_TO_ASSUME_ARN}\", \"secured\": \"false\" }" https://api.bitbucket.org/2.0/workspaces/$WORKSPACE/pipelines-config/variables > /dev/null
}

function deploy_ljhdatalake_foundations_scm() {
    ...
    LJHDATALAKE_AWS_ACCESS_KEY_ID=$(jq -r '.[] | select(.ParameterKey=="ljhdatalake_aws_access_key_id") | .ParameterValue' ${SCM_DIR}/parameters.json)
    LJHDATALAKE_AWS_SECRET_ACCESS_KEY=$(jq -r '.[] | select(.ParameterKey=="ljhdatalake_aws_secret_access_key") | .ParameterValue' ${SCM_DIR}/parameters.json)
    LJHDATALAKE_AWS_SESSION_TOKEN=$(jq -r '.[] | select(.ParameterKey=="ljhdatalake_aws_session_token") | .ParameterValue' ${SCM_DIR}/parameters.json)
    LJHDATALAKE_AWS_ROLE_TO_ASSUME_NAME=$(jq -r '.[] | select(.ParameterKey=="ljhdatalake_aws_role_to_assume_name") | .ParameterValue' ${SCM_DIR}/parameters.json)
    LJHDATALAKE_AWS_ROLE_TO_ASSUME_ARN=$(jq -r '.[] | select(.ParameterKey=="ljhdatalake_aws_role_to_assume_arn") | .ParameterValue' ${SCM_DIR}/parameters.json)
