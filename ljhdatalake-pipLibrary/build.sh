...
do
    ...
    if [ -f "./requirements.txt" ]; then
        ...
        dir_name=$(echo "${dir//.\/}")
        echo "Uploading Lambda Layer as ljhdatalake-$team_name-$dir_name..."
        
        set +e
        layer=$(aws lambda publish-layer-version --layer-name ljhdatalake-$team_name-$dir_name --description "Contains the libraries specified in requirements.txt" --compatible-runtimes "python3.6" "python3.7" "python3.8" --zip-file fileb://./layer.zip)
        status=$?
        set -e
        ...
        paramname=$(printf '/LJHDATALAKE/Lambda/%s/%s' $team_name $dir_name)
        ...
        artifacts_bucket=$(aws ssm get-parameter --name /LJHDATALAKE/S3/ArtifactsBucket --query "Parameter.Value" --output text)
