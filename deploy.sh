...
DIRNAME=$(pwd)
declare -a REPOSITORIES=("ljhdatalake-foundations" "ljhdatalake-team" "ljhdatalake-pipeline" "ljhdatalake-dataset" "ljhdatalake-datalakeLibrary" "ljhdatalake-pipLibrary" "ljhdatalake-stageA" "ljhdatalake-stageB" "ljhdatalake-utils")
...
usage () { echo "
    ...
    -d -- Demo mode
    -f -- Deploys LJHOOKER DATALAKE Foundations
...
function deploy_ljhdatalake_foundations()
{
...
if $fflag
then
    echo "Deploying LJHDATALAKE foundational repositories..." >&2

    if $xflag ; then
        echo "External SCM deployment detected: ${SCM}"
        deploy_ljhdatalake_foundations_scm
    else
        deploy_ljhdatalake_foundations
    fi
    STACK_NAME=ljhdatalake-cicd-team-repos
    aws cloudformation create-stack \
        --stack-name ${STACK_NAME} \
        --template-body file://${DIRNAME}/ljhdatalake-cicd/template-cicd-team-repos.yaml \
        --tags Key=Framework,Value=ljhdatalake \
...
if $oflag
then
    STACK_NAME=ljhdatalake-cicd-shared-foundations-${ENV}
    aws cloudformation deploy \
        --stack-name ${STACK_NAME} \
        --template-file ${DIRNAME}/ljhdatalake-cicd/template-cicd-shared-foundations.yaml \
        --parameter-overrides \
            pEnvironment="${ENV}" \
            pChildAccountId="${CHILD_ACCOUNT}" \
        --tags Framework=ljhdatalake \
        --capabilities "CAPABILITY_NAMED_IAM" "CAPABILITY_AUTO_EXPAND" \
        --region ${REGION} \
        --profile ${DEVOPS_PROFILE}
    echo "Waiting for stack to be created ..."
    ...
    then
        DEVOPS_ACCOUNT_KMS=$(aws ssm get-parameter --name /LJHDATALAKE/KMS/${ENV}/CICDKeyId --region ${REGION} --profile ${DEVOPS_PROFILE} --query "Parameter.Value" --output text)
        for REPOSITORY in "${REPOSITORIES[@]}"
        do
            # Currently the tests focus on cfn-lint and cfn_nag scans. Ignoring repositories
            # that do not have these templates.

            if [ "$REPOSITORY" != "ljhdatalake-datalakeLibrary" ] && [ "$REPOSITORY" != "ljhdatalake-pipLibrary" ]
            then
                ...
                #       if [ "$REPOSITORY" == "ljhdatalake-team" ]
                #       then
                ...
                aws cloudformation deploy \
                    --stack-name "${STACK_NAME}" \
                    --template-file ${DIRNAME}/ljhdatalake-cicd/template-codecommit-pr-check.yaml \
                    --parameter-overrides \
...
then
    ...
    DEVOPS_ACCOUNT_KMS=$(aws ssm get-parameter --name /LJHDATALAKE/KMS/${ENV}/CICDKeyId --region ${REGION} --profile ${DEVOPS_PROFILE} --query "Parameter.Value" --output text)
    STACK_NAME=ljhdatalake-cicd-child-foundations
    aws cloudformation deploy \
        --stack-name ${STACK_NAME} \
        --template-file ${DIRNAME}/ljhdatalake-cicd/template-cicd-child-foundations.yaml \
        --parameter-overrides \
            pDemo="${DEMO}" \
            pEnvironment="${ENV}" \
